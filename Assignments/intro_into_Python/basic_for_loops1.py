# Basic - Print all integers from 0 to 150. Hint:use a for loop and range
for i in range(0,151,1):
    print(i, ",", end = '')

print("*"*50)

# Multiples of Five - Print all the multiples of 5 from 5 to 1,000
for i in range(5,1001,5):
    print(i, ",", end = '')

print("*"*50)

# Counting, the Dojo Way - Print integers 1 to 100. If divisible by 5, print "Coding" instead.
#If divisible by 10, print "Coding Dojo".
for i in range(1,101):
    if i % 10 == 0:
        print("Coding Dojo")
    elif i % 5 == 0:
        print("Coding")

print("*"*50)

# Whoa. That Sucker's Huge - Add odd integers from 0 to 500,000, and print the final sum.
sum =0
for i in range(0,500001):
    if i % 2 != 0:
        sum += i
print("The sum odd integers from 0 to 500,000 is ", sum)
print("*"*50)

# Countdown by Fours - Print positive numbers starting at 2018, counting down by fours.
i = 2018
while i > 0:
    print(i, "," , end = '')
    i -= 4

print("*"*50)

# Flexible Counter (optional) - Set three variables: lowNum, highNum, mult. Starting at lowNum and
# going through highNum, print only the integers that are a multiple of mult.
# For example, if lowNum=2, highNum=9, and mult=3, the loop should print 3, 6, 9 (on successive lines)

def multiply3 (lowNum, highNum, mult):
    for i in range(lowNum,highNum+1,mult):
        print(i)

print("Multiples of 3 from 0 to 9 are ")
multiply3(lowNum =0 , highNum = 9, mult=3)
